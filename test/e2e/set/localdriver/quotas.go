...
var _ = g.Describe("XFS Quotas", func() {
	...
	// Framework also registers an AfterEach which makes f unusable. Any code using
	...
	cleanup := func() {
		...
		kubeframework.ExpectNoError(errors.NewAggregate(errs), "while cleaning up resource")
	}
	...
	g.It("should fail to write data when volume quota is already reached", func() {
		...
		if err == nil {
			kubeframework.Failf("Expected error during write exceeding quota")
		}
		if !strings.Contains(stderr, "quota exceeded") && !strings.Contains(stderr, "No space left on device") {
			kubeframework.Failf("Expected error in stderr of command writing to location exceeding quota, got %s", stderr)
		}
	})
})
