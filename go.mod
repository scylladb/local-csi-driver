...
// v1.13.0 removes the volume condition API `k8s.io/kubernetes/pkg/volume/csi` compiles
// against. Replaced rather than only required, as `require` is just a lower bound.
...
// use `k8s.io/kubernetes` as a library (see https://github.com/kubernetes/kubernetes/issues/79384).
// They can be updated with `make update-k8sio-gomod-replace`.
replace (
