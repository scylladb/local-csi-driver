...
func (o *LocalDriverOptions) Validate() error {
	...
	if _, _, err := o.parseEndpoint(o.Endpoint); err != nil {
...
func (o *LocalDriverOptions) makeListener(endpoint string) (net.Listener, func() error, error) {
	proto, addr, err := o.parseEndpoint(endpoint)
	if err != nil {
		return nil, nil, err
	}
	...
	if proto == "unix" {
		...
		return nil, nil, fmt.Errorf(`unsupported protocol type %q, only "unix" is supported`, proto)
	}

	l, err := net.Listen(proto, addr)
	if err != nil {
		return nil, nil, fmt.Errorf("can't listen on addr %q using %q protocol: %w", addr, proto, err)
	}

	return l, cleanup, err
}

func (o *LocalDriverOptions) parseEndpoint(endpoint string) (string, string, error) {
