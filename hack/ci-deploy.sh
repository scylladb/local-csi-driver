...
cp ./example/storageclass.yaml "${deploy_dir}/"

for f in $( find "${deploy_dir}"/ -type f -name '*.yaml' ); do
    sed -i -E -e "s~docker.io/scylladb/xfs-csi-driver(:|@sha256:)[^ ]*~${DRIVER_IMAGE_REF}~" "${f}"
done

kubectl_create -f "${deploy_dir}"/storageclass.yaml
...
kubectl -n xfs-csi-driver rollout status --timeout=5m daemonset.apps/xfs-csi-driver
